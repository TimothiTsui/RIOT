...
static int ph_oem_init_test(const ph_oem_t *dev)
{
    ...
    /* Register read test */
    if ((i2c_status = i2c_read_regs(I2C, ADDR, PH_OEM_REG_DEVICE_TYPE,
                                    &reg_data, 1, 0x0)) < 0) {
        DEBUG("\n[ph_oem] init test - error: unable to read reg %x\n",
    ...
    if (reg_data != PH_OEM_DEVICE_TYPE_ID) {
        DEBUG("[ph_oem] init test - error: the attached device is not a pH OEM "
    ...
    if (gpio_init(EN_PIN, GPIO_OUT) < 0) {
        DEBUG("[ph_oem] init test - initializing enable gpio pin failed");
...
int ph_oem_read_firmware_version(const ph_oem_t *dev, int16_t *firmware_version)
{
    ...
    if (i2c_read_reg(I2C, ADDR, PH_OEM_REG_FIRMWARE_VERSION,
                     firmware_version, 0x0) < 0) {
        DEBUG("[ph_oem debug] Reading firmware version failed.");
        i2c_release(I2C);
...
static int ph_oem_unlock_address_reg(ph_oem_t *dev)
{
    ...
    if (reg_value != 0x00) {
        DEBUG("[ph_oem debug] Failed at unlocking I2C address register...");
        i2c_release(I2C);
...
int ph_oem_set_i2c_address(ph_oem_t *dev, uint8_t addr)
{
    ...
    if (i2c_write_reg(I2C, ADDR, PH_OEM_REG_ADDRESS, addr, 0x0) < 0) {
        DEBUG("[ph_oem debug] Setting I2C address to %x failed", addr);
        i2c_release(I2C);
        return PH_OEM_WRITE_ERR;
    }
...
int ph_oem_set_device_state(const ph_oem_t *dev, ph_oem_device_state_t state)
{
    ...
    if (i2c_write_reg(I2C, ADDR, PH_OEM_REG_HIBERNATE, state, 0x0) < 0) {
        DEBUG("[ph_oem debug] Setting device state to %d failed", state);
        i2c_release(I2C);
        return PH_OEM_WRITE_ERR;
    }
    i2c_release(I2C);
...
static int ph_oem_new_reading_available(const ph_oem_t *dev)
{
    ...
    do {
        if (i2c_read_reg(I2C, ADDR, PH_OEM_REG_NEW_READING,
            ...
            DEBUG("[ph_oem debug] Failed at reading PH_OEM_REG_NEW_READING\n");
            i2c_release(I2C);
            return PH_OEM_READ_ERR;
        }
    ...
    if (i2c_write_reg(I2C, ADDR, PH_OEM_REG_NEW_READING, 0x00, 0x0) < 0) {
        DEBUG("[ph_oem debug] Resetting PH_OEM_REG_NEW_READING failed\n");
        i2c_release(I2C);
        return PH_OEM_WRITE_ERR;
    }
    i2c_release(I2C);
...
int ph_oem_clear_calibration(const ph_oem_t *dev)
{
    ...
    i2c_acquire(I2C);
    DEBUG("[ph_oem debug] Clearing calibration \n");
                                        ...
                                        uint16_t calibration_value)
{
    ...
    DEBUG("[ph_oem debug] Calibrating device to pH raw: %d...",
          calibration_value);
    i2c_write_regs(I2C, ADDR, PH_OEM_REG_CALIBRATION_BASE, &reg_value, 4, 0);

    /* Calibration is critical, so check if written value is in fact correct */
    i2c_read_regs(I2C, ADDR, PH_OEM_REG_CALIBRATION_BASE, &reg_value, 4, 0);
                           ...
                           ph_oem_calibration_option_t option)
{
    ...
    DEBUG("[ph_oem debug] Calibration for option 0x%02x...", option);
    i2c_status = i2c_write_reg(I2C, ADDR, PH_OEM_REG_CALIBRATION_REQUEST,
                               option, 0);
    if (i2c_status < 0) {
        DEBUG("FAILED]\n");
        return PH_OEM_WRITE_ERR;
...
int ph_oem_read_calibration_state(const ph_oem_t *dev, int16_t *calibration_state)
{
    ...
    if (i2c_read_reg(I2C, ADDR, PH_OEM_REG_CALIBRATION_CONFIRM, calibration_state, 0) < 0) {
        DEBUG("[ph_oem debug] Failed at reading calibration confirm register\n");
        i2c_release(I2C);
        return PH_OEM_READ_ERR;
    }
    i2c_release(I2C);
