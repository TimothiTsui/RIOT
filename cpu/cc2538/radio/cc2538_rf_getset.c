...
static const uint8_t power_lut[NUM_POWER_LEVELS] = {0, /**< -24 dBm */
7, /**< -23 dBm */
15, /**< -22 dBm */
22, /**< -21 dBm */
29, /**< -20 dBm */
...
void cc2538_set_chan(unsigned int chan){
    ...
    /* Disable radio module if it is enabled before changing channel.
     * Otherwise it throughs an error:
     * isr_rfcoreerr(): RFERRF=0x01 and
     * isr_rfcoreerr(): NLOCK detected!*/
...
void cc2538_set_state(cc2538_rf_t *dev, netopt_state_t state){
    ...
    case NETOPT_STATE_IDLE:
        if(!cc2538_is_on()) {
            cc2538_on();
            RFCORE_WAIT_UNTIL(
                    RFCORE->XREG_FSMSTAT0bits.FSM_FFCTRL_STATE > FSM_STATE_RX_CALIBRATION);
        }
        dev->state = state;
        break;
